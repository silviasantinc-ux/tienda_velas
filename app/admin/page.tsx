...
export default function AdminPanel() {
  ...
  const exportarCSV = async () => {
    ...
    const cabecera = ['Nombre', 'Nombre CA', 'Variante (ES)', 'Variante (CA)', 'Categoría', 'Categoría CA', 'Precio (€)', 'Stock', 'Etiqueta', 'Peso (g)', 'Alto (cm)', 'Diámetro (cm)', 'Activo']
    ...
    for (const p of prods) {
      ...
      if (vars.length === 0) {
        filas.push([p.nombre, p.nombre_ca ?? '', '', '', p.categoria, p.categoria_ca ?? '', p.precio.toFixed(2), p.stock, p.badge ?? '', p.peso_gr ?? '', p.alto_cm ?? '', p.ancho_cm ?? '', activo])
      } else {
        for (const v of vars) {
          const precio = (p.precio + (v.precio_extra ?? 0)).toFixed(2)
          filas.push([p.nombre, p.nombre_ca ?? '', v.nombre, v.nombre_ca ?? '', p.categoria, p.categoria_ca ?? '', precio, v.stock, p.badge ?? '', p.peso_gr ?? '', p.alto_cm ?? '', p.ancho_cm ?? '', activo])
        }
      }
    }
    ...
    <div className="min-h-screen bg-[#f6f4f1]">
      ...
      <header className="bg-white border-b border-[#e0ddd8] px-8 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-6">
          <Link href="/admin/configuracion" className="flex items-center gap-2 text-[10px] uppercase tracking-widest text-[#767676] hover:text-[#1b1b1b] transition-colors">
          ...
          <Link href="/admin/colores" className="flex items-center gap-2 text-[10px] uppercase tracking-widest text-[#767676] hover:text-[#1b1b1b] transition-colors">
