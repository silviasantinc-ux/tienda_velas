...
export default function AdminCategorias() {
  ...
  const guardar = async () => {
    ...
    if (nuevo) {
      ...
      const { error: err } = await supabase.from('categorias').update({ nombre: form.nombre, nombre_ca: form.nombre_ca }).eq('id', form.id)
      if (err) { setError(err.message); return }
    }
  ...
  const iniciarEdicion = (c: Categoria) => { setEditando(c); setNuevo(false); setForm({ id: c.id, nombre: c.nombre, nombre_ca: c.nombre_ca ?? '' }); setError(null) }
            ...
            <tbody>
              {cargando ? (
                ...
                <tr key={c.id} className="border-b border-[#f0ede8] hover:bg-[#faf9f7] transition-colors">
                  <td className="px-6 py-4 text-[11px] text-[#999] font-mono">{c.id}</td>
                  <td className="px-4 py-4 text-sm text-[#1b1b1b]">{c.nombre}</td>
                  <td className="px-4 py-4 text-sm text-[#666]">{c.nombre_ca ?? '—'}</td>
