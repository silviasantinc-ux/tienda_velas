...
export default function AdminColores() {
  ...
  const cargar = async () => {
    setCargando(true)
    const { data } = await supabase.from('colores').select('*').order('orden').order('nombre')
    setItems(data ?? [])
    setCargando(false)
  }
  ...
  const guardar = async () => {
    ...
    if (nuevo) {
      const { error: err } = await supabase.from('colores').insert(payload)
      if (err) { setError(err.message); return }
    } else {
      const { error: err } = await supabase.from('colores').update(payload).eq('id', form.id)
      if (err) { setError(err.message); return }
    }
  ...
  const toggleActivo = async (c: Color) => {
    await supabase.from('colores').update({ activo: !c.activo }).eq('id', c.id)
    setItems((prev) => prev.map((x) => x.id === c.id ? { ...x, activo: !c.activo } : x))
  }

  const iniciarEdicion = (c: Color) => {
    setEditando(c); setNuevo(false)
    setForm({ id: c.id, nombre: c.nombre, nombre_ca: c.nombre_ca ?? '', activo: c.activo, orden: c.orden.toString() })
    setError(null)
  }
    ...
    <div className="min-h-screen bg-[#f6f4f1]">
      <header className="bg-white border-b border-[#e0ddd8] px-8 py-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <span className="text-[11px] uppercase tracking-widest font-medium text-[#1b1b1b]">Colores para personalización</span>
        </div>
        ...
        {(nuevo || editando) && (
          <div className="bg-white border border-[#e0ddd8] p-6 space-y-4">
            <h2 className="text-[11px] uppercase tracking-widest font-medium text-[#1b1b1b]">
              {nuevo ? 'Nuevo color' : 'Editar color'}
            </h2>
          ...
          <table className="w-full">
            ...
            <tbody>
              {cargando ? (
                ...
                <tr><td colSpan={5} className="text-center py-10 text-sm text-[#767676]">Sin colores. Añade el primero.</td></tr>
              ) : items.map((c) => (
                <tr key={c.id} className="border-b border-[#f0ede8] hover:bg-[#faf9f7] transition-colors">
                  <td className="px-6 py-3 text-sm text-[#1b1b1b]">{c.nombre}</td>
                  <td className="px-4 py-3 text-sm text-[#666]">{c.nombre_ca ?? '—'}</td>
                  <td className="px-4 py-3 text-sm text-[#666]">{c.orden}</td>
                  <td className="px-4 py-3">
                    <button onClick={() => toggleActivo(c)} title={c.activo ? 'Desactivar' : 'Activar'}
                      className={c.activo ? 'text-[#7d5d24] hover:text-[#ccc] transition-colors' : 'text-[#ccc] hover:text-[#7d5d24] transition-colors'}>
